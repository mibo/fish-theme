function fish_prompt
	...
	set directory (set_color green)(echo $PWD)(set_color normal)
	...
	set git (__fish_git_prompt " ($color_reset%s")

	#echo -s $user (set_color yellow)'@' $machine ':' $directory $git')'
	#echo -s 'work' (set_color --bold yellow)'@' $machine ':' $directory $git')'

	echo "" # just for an additional line break
	echo -sn (set_color brblack) '['(date "+%H:%M:%S")']' (set_color --bold yellow)'@'
	echo -s (set_color normal) $machine ':' $directory $git')'

	if [ $USER = "root" ]
		set caret (set_color red ) "#"
	else
		set caret (set_color --bold green) "\$"
	end

	set cdir (echo $PWD | sed -e "s|^$realhome|~|")
	echo -s (set_color --bold green)(basename $cdir) $caret ' '(set_color normal)
end